...
from models import CustomizedMoEPositionwiseFF
...
def update_topk(model, top_k):
    for name, m in model.named_modules():
        if isinstance(m, CustomNaiveGate_Balance_SMoE) or (
            isinstance(m, CustomNaiveGate_Balance_XMoE)
            or isinstance(m, CustomNaiveGate_Balance_StableMoE)
        ):
            print(f"{name} top_k change: {top_k}")
            m.top_k = top_k
        if isinstance(m , CustomizedMoEPositionwiseFF):
            m.top_k = top_k

def activate_contrastive(model, stage):
    for name, m in model.named_modules():
        if isinstance(m, CustomNaiveGate_Balance_SMoE) or (
            isinstance(m, CustomNaiveGate_Balance_XMoE)
            or isinstance(m, CustomNaiveGate_Balance_StableMoE)
        ):
            print(f"{name} Contrastive change: {stage}")
            m.contrastive = stage
        if isinstance(m , CustomizedMoEPositionwiseFF):
            m.contrastive = stage
...
def _get_scheduler_lr(optimizer, lr_warmup=0, step_size=4000, gamma=0.9):

    return torch.optim.lr_scheduler.StepLR(optimizer, step_size=step_size, gamma=gamma)


def _get_scheduler_linear(
    optimizer, lr_warmup=0, start_factor=1.0, end_factor=0.1, total_iters=30000
):
...
def _get_scheduler_mullinear(
    optimizer, lr_warmup=0,  milestones=[20000,40000], gamma=0.1
):
...
def _get_scheduler_consine(optimizer, lr_warmup=0, max_step=70000, eta_min=0):
    return torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer, max_step, eta_min=eta_min
    )


def _get_scheduler_inv(optimizer, lr_warmup):
    ...
    def lr_lambda(step):
        ...
        if step == 0 and args.warmup_step == 0:
            ...
            return 1.0 / (step**0.5) if step > lr_warmup else step / (lr_warmup**1.5)

    if lr_warmup > 0:
        return torch.optim.optim.lr_scheduler.LambdaLR(optimizer, lr_lambda=lr_lambda)
    return None
...
def get_optimizer_and_scheduler(model, optim_params):
    ...
    scheduler = _get_scheduler_mullinear(
        optimizer=optimizer, lr_warmup=optim_params["lr_warmup"]
    )
